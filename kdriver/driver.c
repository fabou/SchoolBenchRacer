...
int *	Strategy_of_one_who_is_unaware_of_the_existence_of_others(const struct s_map map, const int my_x0, const int my_y0, const int my_vx0, const int my_vy0, const int i_skip_initial_turn)  /*=my_initial_position, my_initial_velocity, do I skip next turn*/){	/*ToDo: shorter name.*/
	...
	/*how to encode map? see representation.h*/
	...
	/* I may want to use one of these
		a.) A function to determine reliably, for a given state, a minimum (Lbound) for how many turns it will take to go from here to a victory ,
			(wiki: "admissible heuristic estimate" of the distance to a goal from this state + what is on the turn counter)
			this + what is on the turn counter = a lower bound for how many turns the best state 'below' this state, will have, on its turn counter
		This function, for a state, can take into account:
			the state (am I moving backwards?),
			the information gathered by the search up to that point
			"any extra knowledge about the problem"
	*/
