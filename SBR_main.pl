...
overdraw_next_pos(@moeglichkeiten_alt);
...
my ($white,$trackcolor,$trackgridcolor,$bgcolor,$fgcolor,$car1color,$car2color,$car3color);
my ($carpos_x,$cartext,$carfont,$overtextgrid_inner);
sub config_map_screen {
$white = new SDL::Color(-r=>255, -g=>255, -b=>255);#white
...
$carfont = new SDL::TTFont(-name=>"Fonts/Vera.ttf", -size=>12, -bg=>$trackgridcolor, -fg=>$white);
}
sub draw_map {
...
sub draw_cars {
        ...
        $carfont->print($app, $gridpos_x + 2, $gridpos_y + $text_y, $cartext);
 ...
 sub draw_next_pos {
   ...
   foreach (0 .. $#poss) {
     ...
     my $m_pos_x = $relativ_x * $grid_x;
     my $m_pos_y = $relativ_y * $grid_y;
     $posfont->print($app, $m_pos_x + 2, $m_pos_y + 2, $postext);
     $app->sync();
   }
   @moeglichkeiten_alt = @poss
}
 sub overdraw_next_pos {
   my (@poss_alt) = @_;
   
   foreach (0 .. $#poss_alt) {

     my ( $relativ_y, $relativ_x) = (${$poss_alt[$_]}[0], ${$poss_alt[$_]}[1]);
     my $m_pos_x = $relativ_x * $grid_x;
     my $m_pos_y = $relativ_y * $grid_y;
     my $map_pos_poss = $RaceTrack[$relativ_x][$relativ_y];
     #if ($map_pos_poss == 1){
     $overcargrid = new SDL::Rect(-width=>$grid_x - 2, -height=>$grid_y - 2,-x=>$m_pos_x + 2, -y=>$m_pos_y + 2);
        $app->fill($overcargrid, $trackgridcolor);
     $app->sync();
     #}
   }
   track_cars();
